...
#' It either returns TRUE if all variables conform to the rules or stops execution in case one of the conditions is not met.
#' 
...
#' @param indexMap A numeric 2D array-like. Must always be validated simultaneously with \code{imageStack}
#' @param fix.gaussian.blur A logical parameter, or one which can be coerced to a logical value
#' @param blur.size An integer value
#' @param return.all A logical parameter, or one which can be coerced to a logical value
#' 
...
validateVariables = function(imageStack, image, w_x, w_y, smoothing, brushShape, projType, interpolation, 
  ...
  if(!missing(contrastStack)) {
    ...
    if(length(dim(contrastStack)) != 3) stop("'contrastStack' must be a 3D array")
    if(min(contrastStack) < 0) stop("All values of 'contrastStack' must be >= 0")
  }
  ...
  if(!missing(smoothing)) {
    ...
    if(as.integer(smoothing) != smoothing) stop("'smoothing' must be an integer")
    if(smoothing < 0) stop("'smoothing' must be >= 0")
  }
  
  if(!missing(interpolation)) {
    if(!is.numeric(interpolation)) stop("'interpolation' must be an integer")
    if(as.integer(interpolation) != interpolation) stop("'interpolation' must be an integer")
  ...
  if(!missing(indexMap)) {
    if(missing(imageStack)) stop("Validation of 'indexMap' only makes sense if 'imageStack' is also being validated")
    if(!is.numeric(indexMap)) stop("'indexMap' must be numeric")
    if(length(dim(indexMap)) != 2) stop("'indexMap' must have two dimensions: (height, width)")
    if(!all(dim(imageStack)[c(1,2)] == dim(indexMap))) stop("The spatial dimensions of 'imageStack' and 'indexMap' must be identical")
    if(min(indexMap) < 1) stop("All values of 'indexMap' must be >= 1")
    if(max(indexMap) > dim(imageStack)[3]) stop("All values of 'indexMap' must be <= the number of image stacks (", dim(imageStack)[3], ")")
  }
  
  if(!missing(fix.gaussian.blur)) {
    fix.gaussian.blur.log = as.logical(fix.gaussian.blur)
    if(is.na(fix.gaussian.blur.log)) stop("'fix.gaussian.blur' must be a TRUE/FALSE or must be able to be coerced to TRUE/FALSE")
  }
  
  if(!missing(blur.size)) {
    if(!is.numeric(blur.size)) stop("'blur.size' must be an integer")
    if(as.integer(blur.size) != blur.size) stop("'blur.size' must be an integer")
    if(blur.size < 0) stop("'blur.size' must be >= 0")
  }
  
  if(!missing(return.all)) {
    return.all.log = as.logical(return.all)
    if(is.na(return.all.log)) stop("'return.all' must be a TRUE/FALSE or must be able to be coerced to TRUE/FALSE")
  }
...
#' 
#' @description Calculate the local contrast of an input image for every pixel of the image over a given window centered on that pixel.
#' 
...
#' a given pixel and the variance of the intensity values within this window are determined via Var = E[X^2] - (E[X])^2.
#' 
#' The \code{brushShape} indicates the shape of the window over which to calculate the variance. Depending on the symmetry of the objects being imaged, the window
#' shape may have a significant impact on the quality of the projection.
#' 
#' @return A 2D matrix with the same dimensions as \code{image}, which shows the local contrast at each corresponding pixel of \code{image}.
#' 
#' @author Jan Sauer
#' @keywords array
#' 
...
calcContrast = function(image, w_x, w_y, brushShape = "box", validate = TRUE) {
  ...
  # Validate arguments
  if(validate) valid = validateVariables(image = image, w_x = w_x, w_y = w_y, brushShape = brushShape)
  
  f = NULL
  if(brushShape == "box") f = matrix(1 / ((2*w_x + 1)*(2*w_y + 1)), nrow = (2*w_y + 1), ncol = (2*w_x + 1))
  if(brushShape == "disc") {
    f = makeBrush(size = 2*w_x + 1, shape = "disc", step = TRUE)
    f = f / sum(f)
  }
  imgSquared = image**2
  contrast = filter2(x = Image(imgSquared), filter = f, boundary = "replicate") - filter2(x = Image(image), filter = f, boundary = "replicate")**2
  return(contrast)
}
...
getContrastStack = function(imageStack, w_x, w_y, brushShape = "box", validate = TRUE) {
  ...
  # Validate arguments
  if(validate) valid = validateVariables(imageStack = imageStack, w_x = w_x, w_y = w_y, brushShape = brushShape)
...
#' @describeIn contrastProjection Get the index map (with or without smoothing) which indicates the layer corresponding to the highest contrast for each pixel in the \eqn{(x,y)}-plane
#' @export
getIndexMap = function(contrastStack, smoothing = 0, validate = TRUE) {
  ...
  # Validate arguments
  if(validate) valid = validateVariables(contrastStack = contrastStack, smoothing = smoothing)
...
#' 
#' @description Projects a z-stack of 2D images according to the highest local contrast. Optionally, median smoothing can be applied to the resulting projection index map prior to the projection itself.
#' 
...
#' Then, the z-layer with the highest local contrast is determined for each pixel in the \eqn{(x,y)}-plane, resulting in an index map with the same spatial 
#' dimensions as the input images. This index map can then be smoothed with a median filter if desired. \code{getIndexMap} returns this index map. Lastly, the
#' image stack is projected into the \eqn{(x,y)}-plane using this index map to determine which z-layer to use at every pixel. \code{contrastProjection} returns
#' this fully projected image.
#' 
#' The \code{brushShape} indicates the shape of the window over which to calculate the variance. Depending on the symmetry of the objects being imaged, the window
#' shape may have a significant impact on the quality of the projection.
#' 
#' If an object lies in several focal plains then the projection may include some artifical boundaries at the edges of the regions in each focal plain. Linear interpolation
#' between the two layers at their boundaries serves to eliminate this problem. The \code{interpolation} size gives the size of the kernel to use for blurring the boundaries 
#' between individual regions of the index map. The projection values at these boundaries are then interpolated based on the non-integer values on the index maps. For example,
#' if a pixel on the index map has the value 7.25, then the projected value at this pixel is 75% of the intensity in z-layer 7 and 25% of the intensity in layer 8.
#' 
#' If a very bright object lies on a dark background, then the gaussian blurring of the unfocused image stacks can create a brighter ring structure around this object. Fixing this involves
#' Voronoi propagation into the regions directly surrounding bright objects. This correction only makes sense if there is a clear differentiation between fore- and background 
#' in the image. A perfect segmentation is unnecessary as the rings will only appear around exceptionally bright objects, which are easy to segment.
#' 
#' @return 
#'  \item{contrastProjection}{A 2D matrix corresponding to the maximum contrast projection of \code{imageStack}}
#'  \item{getIndexMap}{A 2D matrix indicating the z-layer with the maximum contrast at every pixel in the \eqn{(x,y)-plane} of \code{imageStack}}
#'  \item{getContrastStack}{a 3D array corresponding to the contrast map for every image of \code{imageStack}}
#'  \item{projection_fromMap}{A 2D matrix corresponding to the maximum contrast projection of \code{imageStack}}
#'  
#' @author Jan Sauer
#' @keywords array
#' 
...
#' @export
contrastProjection = function(imageStack, w_x, w_y = NULL, smoothing = 0, brushShape = "box", 
  ...
  # Validate arguments
  valid = validateVariables(imageStack = imageStack, w_x = w_x, w_y = w_y, smoothing = smoothing, 
                            brushShape = brushShape, fix.gaussian.blur = fix.gaussian.blur, blur.size = blur.size,
                            return.all = return.all)
  
  # Get the contrast stack
  contrast_stack = getContrastStack(imageStack = imageStack, w_x = w_x, w_y = w_y, brushShape = brushShape, validate = FALSE)
  
  # Get the index map
  index_map = getIndexMap(contrastStack = contrast_stack, smoothing = smoothing, validate = FALSE)
  
  # Fix the false blur around bright objects if desired
  if(fix.gaussian.blur) index_map = fixGaussianBlur(imageStack = imageStack, indexMap = index_map, blur.size = blur.size)
  
  # Project the stack according to the index map
  proj = projection_fromMap(imageStack = imageStack, indexMap = index_map, interpolation = interpolation)
  
  if(return.all == FALSE) return(proj)
  if(return.all == TRUE) return(list(Contrast.Stack = contrast_stack, Index.Map = index_map, Projection = proj))
}

#' @describeIn contrastProjection Get the index map (with or without smoothing) which indicates the layer corresponding to the highest contrast for each pixel in the \eqn{(x,y)}-plane
#' @export
projection_fromMap = function(imageStack, indexMap, interpolation = 0, validate = TRUE) {
  # Check that all necessary arguments are present
  if(missing(imageStack)) stop("'imageStack' is missing")
  if(missing(indexMap)) stop("'indexMap' is missing")
  
  # Validate variables
  if(validate) valid = validateVariables(imageStack = imageStack, indexMap = indexMap, interpolation = interpolation)
  ...
  if(interpolation > 0) {
    ...
    selec_ind_low = expand.grid(1:nrow(indexMap_blurred_low), 1:ncol(indexMap_blurred_low))
    low_proj = matrix(imageStack[cbind(selec_ind_low$Var1, selec_ind_low$Var2, as.vector(indexMap_blurred_low))], nrow(indexMap_blurred_low), ncol(indexMap_blurred_low))
    
    indexMap_blurred_high = matrix(ceiling(indexMap_blurred), nrow = nrow(indexMap_blurred), ncol = ncol(indexMap_blurred))
    selec_ind_high = expand.grid(1:nrow(indexMap_blurred_high), 1:ncol(indexMap_blurred_high))
    high_proj = matrix(imageStack[cbind(selec_ind_high$Var1, selec_ind_high$Var2, as.vector(indexMap_blurred_high))], nrow(indexMap_blurred_high), ncol(indexMap_blurred_high))
    
    high_proj_contribution = (indexMap_blurred - indexMap_blurred_low)
    proj = (high_proj_contribution * high_proj) + ((1 - high_proj_contribution) * low_proj)
  } else {
    selec.ind = expand.grid(1:nrow(indexMap), 1:ncol(indexMap))
    proj = matrix(imageStack[cbind(selec.ind$Var1, selec.ind$Var2, as.vector(indexMap))], nrow(indexMap), ncol(indexMap))
  }
...
#' 
#' @description Perform an intensity projection of a stack of images. The type of projection depends on the \code{projType}
#' 
#' @param imageStack A numeric 3D array-like which should ne projected. The dimensions should be (spatial_1, spatial_2, numer_of_images)
#' @param projType A string indicating the type of projection. Defaults to "max" and can take on the following values: \code{"max", "min", "mean", "median", "sd", "sum"}
#' 
...
#' 
#' @return A 2D matrix corresponding to the maximum intensity projection of \code{imageStack}
#' 
#' @author Jan Sauer
#' @keywords array
#' 
...
#' 
#' @description Fix the contrast projection error around very bright objects on a dark background
#' 
#' @param imageStack A numeric 3D array-like which should ne projected. The dimensions should be (spatial_1, spatial_2, numer_of_images)
#' @param indexMap A custom index map according to which the image stack is projected. The values must be integers between 1 and the number of layers in \code{imageStack}
...
#' contrast in the regions directly outside of bright foreground objects than the actual background. This leads to ring shapes around the bright foreground which need to
#' be removed. This is done by detecting bright objects with a primitive, intensity-based segmentation method (Otsu-thresholding) and determining a region around the
#' foreground objects in which the gaussian blurring is visible. In this region, instead of using the determined z-indices for the projection, a Voronoi propagation starting 
#' from the closest foreground pixels is performed to ensure that the same z-layer is used for the nearby background as for the foreground.
#' 
...
fixGaussianBlur = function(imageStack, indexMap, blur.size, validate = TRUE) {
  ...
  if(validate) valid = validateVariables(imageStack = imageStack, indexMap = indexMap, blur.size = blur.size)
  ...
  bin_img_overlay = bin_img_overlay > otsu(x = bin_img_overlay, range = range(bin_img_overlay))
